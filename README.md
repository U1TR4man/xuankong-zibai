...
[最佳時窗 Ranking V1](docs/time-window-ranking-v1-authoritative-rules.md) 已完成：尋星結果可切換「依時間／依日課時課」，**預設仍是依時間**。那是**時間軸**排序（排時段，不排方向），排序鍵只有「可用性 → 日課 → 時課 → 時間」四個，依據是卷三十四「時者，日之用也」。方位軸不在授權範圍，`verdictFor()`／`rankDirections()` 仍讀不到任何 Gate 欄位。
...
- `npm test` — 365 個測試（33 個檔案），涵蓋算法、1,200 點 snapshot、V1 下鑽、V2 Phase 2–6、V2.1／Final UI、資產與鍵盤操作，以及疊盤、尋星 A/B、紫白擇吉四欄橫排、第六輪大月建合流／日白升級、第七輪 9×6 白中殺矩陣、Day Gate V1、Pair 搜尋／學習、URL restore、Search → Chart、字體 glyph 覆蓋與結果 UX 回歸。
...
- 時窗評級只在「依日課時課」排序時顯示；要在預設的依時間模式也顯示，屬另一個 UX 決策。
