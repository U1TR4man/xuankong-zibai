...
第二輪考源加入 `sourceAudit`，分開記錄證據形式、原始使用情境、次序可信度、宮位／方位條件、異文與原頁核對狀態。第三輪建立墓絕、1／6／8／9 支序有氣及月令狀態；第四輪再修正 co-arrival、月暗建與 classical 受剋表。現行規則是一個合格紫白亦可成立；暗建只依月白入中星反推禁修方，不是飛星回本宮。詳見 [第二輪](docs/purple-white-second-round-source-audit.md)、[第三輪](docs/purple-white-third-round-temporal-rules.md)及 [第四輪考源紀錄](docs/purple-white-fourth-round-coarrival-anjian.md)。
...
- `npm test` — 179 個測試，涵蓋算法、1200 點 snapshot、V1 下鑽、V2 Phase 2–6、V2.1 UI／資產／鍵盤操作，以及疊盤、尋星 A/B、紫白擇吉四欄橫排、第四輪 co-arrival／月暗建／classical 受剋殺、Pair 搜尋／學習、URL restore、Search → Chart、字體 glyph 覆蓋與結果 UX 回歸。
