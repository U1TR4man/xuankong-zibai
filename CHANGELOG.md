...
- **修正擇吉九宮：點開方向詳情、下滑關閉後宮格多出一圈邊框。** 全域 `:focus-visible` 的 `outline-offset` 是 **+2px**，焦點框畫在元素**外面**；九宮格子是等分 grid item，那圈框會疊到相鄰格，並在盤緣被 `.grid` 的 `overflow: hidden` 切掉，看起來就是一圈殘缺的多餘邊框。`.overlay-cell` 早已有內側焦點框的處理，**`.selection-cell` 漏了**，現補上同一條規則。
- **為什麼只在關閉 sheet 之後出現**：手指點的當下是 pointer 互動，`:focus-visible` 不匹配；而 `returnFocusSelector` 是**以程式呼叫 `.focus()`**，Safari 對程式性 focus 會套用 `:focus-visible`。焦點框本身不能拿掉——那是鍵盤操作的必要提示——所以改畫在內側而不是移除。
- 新增測試掃描所有 `*Grid.ts`：只要格子是 `button`，就必須有對應的內側 focus ring 規則。已確認拿掉修正後該測試會失敗，日後新增可 focus 的格子若漏掉也會被擋下。

- **撤回一個記錯的考源判斷。** 先前把《完孝錄》卷六兩處「八白飛巳」記為「疑為脫文」——錯了。八白本宮艮＝丑寅，而**寅巳既是三刑（寅巳申）又是六害（寅巳）**，同一格同時中兩條完全正常。
- 實算後另發現該段其實是**可推導的通則**而非四個孤例：原文先立「十二支配八卦」，再說「其九宮星煞，莫不有三刑六害也」。用專案既有的 `isPunishment()`／`isSixHarm()` 從本宮地支推算，四個刑宮例**全部落在推導結果內**。
- 但**仍維持關閉**，不封版的理由更正為兩點：推導給的是**超集**（六白得未亥、八白得戌巳，古本各只取一，原文沒說取哪個）；害宮「一白飛午」與子未相害**對不上**（午是子的沖），須另找版本核對。四空亡另因《協紀》卷七「未可盡以為凶」及《完孝錄》的減半／全然無力屬加權抵消而維持關閉。
- 純文件更正，無程式變更。

### Added

- **尋星結果可切換「依時間 / 依日課時課」排序。** **預設仍是依時間**，與改動前一致——改變既有排序預設屬另一個 UX 決策。依日課時課時改以 tier 分組（日期分組在那個次序下會碎成無意義的片段），每張卡片顯示日課、時課與排除原因，並明示「同一組內按時間先後排列，先後不代表吉凶」。
- **評級只在依日課時課時顯示。** 依時間排序時列表沒有用到這個判定，照樣顯示會讓人把清單讀成「已按吉凶排好」。
- Gate 的中文標籤抽成 `src/ui/gateLabels.ts` 單一來源：方向詳情與尋星結果都要顯示日課／時課，兩份表一旦分歧，同一個 `caution` 會在兩個畫面讀到不同的字。
- 四寬度 QA：320／375／390／430px 全部零溢出（含排序控制、tier 標題與評級列）。字體 subset **未變**，新文案全部落在既有 929 字內。
- 實測 9/1–9/20 離宮流時九紫共 26 個結果分為 12 個 tier，末兩組是「日課 通過 · 時課 不用」與「日課 偏弱 · 時課 不用」——**旺日因時破被壓到最底**，veto 確實不受「日為體」的主從關係約束。
...
- `rankingUse` 在時間層列為**禁用名**（地位同 `yuePo`），時間層用 `timeRankingUse`。日精度時窗的 `hourStatus` 定為 `'not_applicable'`，不重用 `'not_evaluated'`——後者是「尚未評估」，與「此精度無此概念」不同。
...
- `WhiteKillerSourceRef` 只存 stable chapter URL 與頁內 anchor；帶時效簽名的原書影像 URL 不入專案。anchor 刻意選用已在字體 subset 內的字串，逐字引文留在考源文件——否則單為引文就要擴大自帶字體。

- **白中殺 9×6 矩陣逐頁核到完整直證，結論與獨立覆核稿相反。** 該稿主張受剋表只有 1/6/8/9 有證、其餘五星應降為 OCR 重建；實際核對《五要奇書》卷三十八〈脩方要訣〉後，六捷墓、暗建、受剋、穿心、交劍、鬥牛**六列全部與本專案逐格相符**，受剋列還原後為十三字「中／震巽／乾兌／乾兌／震巽／離／離／震巽／坎」，無剩餘無短缺。該頁六列一致地整體右移一位（九紫值排到最前），這個系統性偏移本身就是分欄可還原的內部證據。**無需降級。**
- **順帶解決一個會讓極性反轉的讀法陷阱。** 散文作「一白入中宮**不作**坎暗建殺」，字面像豁免。但《五要奇書》把同句式對九星全列（「一白入中不作坎……九紫入中不作離」），若是豁免則整條暗建殺不存在，與同頁「切忌動作」矛盾。故「不作」＝「不可修作／不得動土」，`作` 是「凡修作年月」的動詞。**本專案極性正確。**
- 核到兩項古本自述的異說：鬥牛「元龜云鬥牛殺者三碧四緑同宫，有此不同，未詳孰是」，以及五黃暗建「五黄入中不作乾坤艮巽」。受剋殺的定義另有「客強主弱」對「客弱主強」的二比一異文。三者並存不修。
- 核到《完孝錄》卷六的限縮：「以上星煞所忌，**特見其例耳**」，還須論刑宮、害宮與四空亡。本專案未實作該層，因此不得把 9×6 矩陣描述為完整的白中殺判定。
- 獨立覆核稿有一項出處錯誤：《類編曆法通書大全》卷二十一並無受剋／暗建的宮位對應，只有 1/6/8/9 的有氣月、入墓支與六殺定義句；該稿把《造命宗鏡集》的內容記到了這一本上。
- 本項全為 read-only 考源，**證據等級與強度一律未改**，年月白中殺仍 active、日時仍 reference-only。

- **核到《協紀》卷四〈義例一〉：「歲德與歲德合，並屬上吉，有宜無忌」。** 先前查不到是因為**維基文庫的《協紀》缺卷一至卷六**，而歲德義例正在卷四；改由識典古籍取得同一四庫本全文。`sui_de` 與 `sui_de_he` 同為 `primary_virtue` 至此是直接引文，不再是「未被列為次吉」的消極推論。同段的剛柔／外事内事之分**不採為分級**——作者自註「選擇家雖未論及此，用者可以意通也」，那是編者發揮而非既存的選擇家規則。月家〈月吉神總論〉的「大吉／次吉」不得類推到歲家。無程式行為變更。
- **五不遇補到完整按語，結論是更不可實作。** 原文為「天干五行氣純，地支五行氣雜，如寅中有火，則遇土日，有相生之理……焉得以土日寅卯時即爲五不遇耶」。作者舉的例是**寅木對土日**，取的是寅的藏干丙火；單一 branch-element 模型會判成木剋土，與原文結論相反。故 detector 維持開啟、自動解除維持關閉。同頁另確認本專案採時干剋日干而不採時支剋日支，與《協紀》「不若神煞起例之善」一致。
- 識典古籍的原書影像 URL 帶時效簽名，**不寫入專案**；考源記錄一律只存 stable chapter URL 與頁內 search anchor。

- 重建字體 subset：本輪的宮名、方位與干支全部早已在 subset 內，因此**沒有新增字元**；反而少了一個「叢」——它隨 `sanDeCongJu` → `sanDeFang` 的更名離開 `src/**`，當時未重建。UI 字元 928 → **927**，cmap 927／glyphs 928，WOFF2 248,584 → **248,116 bytes**，SHA-256 `73a19319a1a381e0c06870ccf60e6a930a00a0e0296675b35beb7a5ea709197d`。source SHA-256 未變且經腳本核對。

- **尋星的宮位改為九宮方盤，不再是下拉選單。** 排列直接沿用 `PALACES` 既有的 `row`／`col`，即洛書 4／9／2、3／5／7、8／1／6，與盤面完全一致；使用者不必再把「離」在腦中翻譯成清單中的第二項。每格顯示宮名與方位，中宮不標方位。飛星選擇器早已是同一個 3×3 grid，兩者現在對齊。
- 中宮仍可選：尋星允許中宮，只有擇吉的八方排序不含中宮。
