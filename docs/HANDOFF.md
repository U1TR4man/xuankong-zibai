...
- **一項自我更正（2026-08-11）**：先前記「八白飛巳在刑宮與害宮重複，疑為脫文」——**誤判，已撤回**。
  八白本宮艮＝丑寅，寅巳**既是三刑又是六害**，同格中兩條正常。實算後另發現該段是
  **可推導的通則**而非孤例（四個刑宮例全部落在 `isPunishment()` 的推導結果內），
  不封版的真正理由改為「推導給超集」與「害宮一白飛午對不上子未相害」。
  詳見考源記錄 §11.7 與第七輪文件〈明確暫緩〉。**刑宮／害宮／空亡仍維持關閉。**
...
| commit | 內容 |
|---|---|
| `de0dfea` | 歲德等級核到直接引文；五不遇支相生確認不可實作 |
| `8b1ab1c` | 白中殺五個古本逐頁核對記錄（考源文件 §11，148 行） |
| （本輪） | 白中殺 audit metadata 升級與異說登記 |

- **維基文庫的《協紀》缺卷一至卷六**。這是先前找不到歲德義例的唯一原因，不是原文沒有。
  卷四改由識典古籍取得同一四庫本。**下一位若要查卷一至卷六，不要再從維基文庫找。**
- 卷四〈義例一〉核到「歲德與歲德合，並屬上吉，有宜無忌」。`sui_de` 與 `sui_de_he`
  同為 `primary_virtue` 由消極推論升為直接引文。同段的剛柔／外事内事**不採為分級**，
  作者自註「選擇家雖未論及此」。月家〈月吉神總論〉的「大吉／次吉」**不得類推到歲家**。
- 卷四同段緊接的「歷例曰：甲年在己，乙年在庚……」講的是**歲幹合**，不是歲德合，
  十干表不同。**不得誤當作歲德合的異文。**
- 五不遇補到完整按語「如寅中有火，則遇土日，有相生之理……焉得以土日寅卯時即爲五不遇耶」。
  原文舉的是寅木對土日、取的是藏干丙火；naive 的支五行判斷會得出相反結論。
  **detector 開、自動解除關**，此結論加強而非放寬。
- **白中殺 9×6 矩陣核到完整直證**：《五要奇書》卷三十八一頁含六列，還原後與本表逐格相符。
  `primarySourceVerified` 升為 `true`，但同時新增 `corroboration:
  'single_complete_witness'`——另外三本受剋列脫格且互不相同（14／4／4 字），
  **不得寫成四本互校**。
- **「不作」是「不可修作」，不是「不算作」。**《五要奇書》把同句式對九星全列，
  若作豁免解則整條暗建殺不存在。極性正確，這個坑已封。
- 新增 `WHITE_KILLER_VARIANTS`（鬥牛元龜三碧四綠同宮、五黃暗建四隅），一律
  `rankingUse: 'disabled'`；新增 `completeness: 'example_set_not_exhaustive'`
  ——《完孝錄》「以上星煞所忌，特見其例耳」，刑宮／害宮／四空亡本專案未實作。
- **強度完全未變**：`confidence` 六列未動，年月白中殺仍 active、日時仍 reference-only，
  八方 verdict 與排序輸出一格未改。
- 識典古籍的原書影像 URL 帶時效簽名，**不入專案**；`WhiteKillerSourceRef` 只存
  stable chapter URL 與 anchor，且 anchor 刻意選用已在字體 subset 內的字串。
- 獨立覆核稿有一項出處錯誤：《類編曆法通書大全》卷二十一**沒有**受剋／暗建的宮位對應，
  只有 1/6/8/9 的有氣月、入墓支與六殺定義句。
...
- **卷三十四是本專案最關鍵的一卷**：〈月吉神總論〉〈月凶神總論〉〈諸家年月日吉凶神附論〉〈制煞要法〉〈四柱法〉〈用日法〉〈用時法〉全部在此卷內。下一位接手者要查 Gate 相關原文，先看卷三十四。
- 六德六張表全部核到原文；歲德／歲德合見卷十四〈年表一〉甲子至癸酉恰涵蓋十天干，天德等四表另有卷二十至三十一〈月表〉逐月佐證，形成第二個固定版本的獨立驗證。`primarySourceVerified` 六項全部升為 `true`。
- **Direction Gate 的歲破與三煞由最弱升為已核對**：卷十四年表逐年列出，十組全數相符；三煞以「巳午未」三個地支列出而非整宮，佐證 partial hit 模型。`evidenceLevel` 由 `C` 升 `A`，`rankingUse` 仍 `disabled`。
- Hour Gate 多數條目核到卷三十四〈用時法〉：「時者日之用也」、時破大凶（子日午時）、時刑次凶（子日卯時）、五不遇次凶（甲日庚午時）、「大事則忌小事可勿論」、旬空「忌出行不忌葬事」、時建、日祿時，以及楊筠松犯五不遇仍用之例。Day Gate 旺相休囚死三分法核到〈用日法〉。
- 月金匱兩處編者按語**都在卷三十四**（〈附論〉按「金匱星今亦不用」、〈火星〉按「然亦有理」並保留四組起例），`source_tension` 措辭確認正確。
- 己／巳 異文範圍比原記錄更廣：《協紀》內部也不一致（卷十四、卷二十一作「己」，卷十七至十九、卷二十五、卷二十九作「巳」），確認為轉錄層形近訛，正字為「己」。
- **兩處落差已登記，均未自行修正**：
  1. 〈用時法〉把**時刑與五不遇並列為「次凶」**，但 `hour-gate-v1-authoritative-rules.md` §4 把五不遇列 C 層、時刑列 D 層，`hourGate.ts` 因此讓五不遇（caution）重於時刑（mixed）。已補記為 §4.3。**precedence 未改，待使用者決定**，宜先補《協紀》卷七〈五不遇時〉再判。
  2. 〈月吉神總論〉只給天德月德「大吉」、兩合德「次吉」，**沒有**給歲德、歲德合等級。兩者同為 `primary_virtue` 只是「未被列為次吉」的消極推論，「並屬上吉」未核到。
- 另發現〈月吉神總論〉明言四德「大能制此方之煞」——原典**確實主張**六德有制煞能力。`virtueCancelsKiller = false` 仍正確，但理由須精確化為「V1 不實作〈制煞要法〉的干支三合納音四法與月令衰旺條件，故不能替使用者宣告已制」。UI 不得寫成「六德不能制煞」，也不得寫成「可制煞」。
- **仍未核到**：《造命宗鏡集》全部條目、《通書大全》卷十二〈三德格〉與「三德叢聚」一詞、《三命通會》四維互合異說。三德叢聚的四組結果由已核對的三張表導出故不受影響，但名詞的古籍地位待核。
- severity 與 `rankingUse` **一律未變**，`hourStatus` 仍 `not_evaluated`，所有表值未改。
