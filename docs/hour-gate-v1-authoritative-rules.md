...
**2026-08-10 補讀卷七後更新**：卷七〈八錄〉作「建合則吉，而**破刑害則凶**也」，
三者並列不分高下；卷三十四把時刑與五不遇並列為次凶。**兩卷都不支持「五不遇重於時刑」。**
本文件 §4 為時刑所引的「次凶，亦輕可」一句，在卷七與卷三十四均未見。

卷七另給了五不遇的解除條件：「然尚須論其所臨之支是否與日干日支相生，如其相生則又不為五不遇也」——
若把解除條件算進去，五不遇反而比時刑更容易被排除。

**2026-08-11 補核到同一按語的完整上下文，並據此確認此條不可實作**（識典古籍固定版本，
search anchor「五不遇時」「如其相生」）：

> 「按，天干五行氣純，地支五行氣雜，如**寅中有火，則遇土日，有相生之理**。
...
1. **本體判準已確認**：同頁作「五不遇時，干尅日干也」，並明言「遁甲隱公歌注則又以本時尅本日之支，
   亦爲五不遇……不若神煞起例之善」。本專案採時干剋日干（十組定局）、**不採時支剋日支**，與《協紀》所取一致。
2. **解除條件反而更不可實作**：作者舉的例正是**寅木對土日**。在單一 branch-element 模型裡
   寅屬木、木剋土，naive 的 `generates(BRANCH_ELEMENT[時支], STEM_ELEMENT[日干])` 會判為「不相生」，
   與原文結論完全相反——原文取的是寅的**藏干丙火**。原文既未給藏干的取用優先級，
   也未說明「與日干、日支相生」是取其一還是兩者兼備，因此任何 deterministic 實作都是自造。

故 V1 維持：**detector 開、自動解除關**（`fiveBuYu` 照常命中並標 `caution`，
詳情文字說明古法另有不作五不遇之論，但本版本不自動解除）。
...
- [x] `daily` / `construction` 模式由使用者如何選擇 —— **2026-08-11 已定案**：
      存於 Settings（`settings.selectionMode`，localStorage 持久化），與日柱換日、年界同組，
      因為三者都是影響計算的解讀選項而非每張盤的臨時切換。方向詳情的時課區另標示當前模式，
      避免使用者看到 `reject` 卻不知它由哪個設定產生。
      **作用範圍限 Hour Gate**：Day Gate 的旺相休囚死與 Direction Gate 的歲破三煞，
      原典都沒有按事情大小分級，擴大適用範圍等於自造判準
- [ ] 日貴人時、日驛馬時
- [ ] 五不遇的「支生扶解除」細則 —— **原文已核到**（卷七：「所臨之支與日干日支相生則又不為五不遇」），但「相生」的判準（對日干？對日支？或兩者）原文未明言，仍不可封版
- [ ] 時家貴登天門、四大吉時、九丑、暗金時
- [ ] 奇門完整擇時、納音、完整神煞庫
